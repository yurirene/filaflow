<section class="w-full">
    ...
    <x-fila.admin-layout :heading="__('Gerenciar consultórios')" class="max-w-5xl">
        <flux:text class="mb-4 text-sm text-zinc-500">
            {{ __('Salas de atendimento na ala. Pacientes encaminhados do guichê entram na fila exclusiva do consultório.') }}
        </flux:text>
        <div class="mb-4">
            <flux:button variant="primary" wire:click="openModal" :disabled="$this->alas->isEmpty()">
                {{ __('Novo consultório') }}
            </flux:button>
        </div>
        <flux:card>
                ...
                <table class="w-full text-left text-sm">
                    <thead>
                        <tr class="border-b border-zinc-200 dark:border-zinc-700">
                            ...
                            <th class="pb-3 font-medium">{{ __('Responsável') }}</th>
                    ...
                    <tbody>
                        @forelse ($this->consultorios as $c)
                            <tr class="border-b border-zinc-100 dark:border-zinc-800" wire:key="consultorio-{{ $c->id }}">
                                ...
                                <td class="py-3">{{ $c->responsavel }}</td>
    ...
    <flux:modal wire:model="showModal" class="max-w-md">
        ...
        <form wire:submit="salvar" class="mt-6 space-y-4">
            ...
            <flux:field>
                ...
                <flux:input wire:model="responsavel" placeholder="{{ __('Ex: Dr. Silva') }}" />
            </flux:field>
            <flux:field>
                <flux:label>{{ __('Serviços permitidos (opcional)') }}</flux:label>
                <flux:text class="mb-2 text-xs text-zinc-500">{{ __('Vazio = qualquer serviço da mesma ala') }}</flux:text>
